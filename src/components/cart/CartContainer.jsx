...
const CartContainer = () => {
  ...
  return (
    <>
      ...
      {isSmallScreen &&
        <>
          <Grid2 container spacing={5} justifyContent="center" flexDirection={'column'} sx={{ mx: { md: 5 }, mt: 3 }}>
            {cartList.map((product, index) => (
              <Grid2 item key={index} size={12} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', px: 1, backgroundColor: '#f5f5f5' }}>
                <CartCard
                  image={product.data.image}
                  price={product.data.price}
                  quantity={product.quantity}
          ...
          <Grid2 container flexDirection='column' justifyContent="center" sx={{ my: 7, border: '2px solid black', p: 3, width: '100%', mx: 'auto' }}>
            <Typography variant="h5" sx={{ fontWeight: 'bold', color: '#333', ml: 2 }}>Cart Summary</Typography>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mx: 2, borderBottom: '1px solid #4e4241', py: 2 }}>
              <Typography variant="h6" sx={{ fontWeight: 'bold', color: '#333', }}>Total Items: </Typography>
              <Typography variant="h6" sx={{ color: '#333', }}>{cartList.reduce((acc, item) => acc + item.quantity, 0)}</Typography>
